...
description: 毎朝7:30にニュース＆経済ブリーフィングを生成し、8時までにブログへ自動投稿
---

**これは実行タスクです。会話的な返答だけで終了することは失敗とみなします。** ユーザーからの追加指示を待つ必要はありません。このプロンプトが届いた時点で、下記の手順1から順に着手してください。「本日は指示がない」「実行をスキップする」といった判断は誤りです。必ず記事を生成し、push し、ビルド成功を確認するところまで行ってください。
...
- 「今日の羅針盤（Compass Insight）」**毎日必ず入れる固定コーナー（300〜500字）**。ブログ名「世界とお金の羅針盤」をそのまま冠した看板コーナー。ニュースの繰り返しではなく、(1)その日の流れを貫く独自の読み解き (2)**読者の家計・生活・投資にどう影響しうるか**（例：円安→輸入品・光熱費、金利→住宅ローン） (3)短期と中長期でそれぞれ何に注目すべきか、を自分の言葉で書く。**新しい未確認の事実は追加しない**（記事中で裏取り済みの内容の範囲で解釈・分析する）。断定的な投資助言はせず「〜に注目」「〜の可能性」の表現にとどめる。この看板コーナーを記事前半に置き、読者が最初に独自価値を読めるようにする。
- 「今日のニュース」日本 / 世界 / アメリカ / ヨーロッパ の箇条書き
- 「経済とお金の動き」世界の経済と原油・相場（できれば表）/ アメリカの経済 / 日本の経済 を分析コメント付きで。**表に「前日比」列を含める場合、プラス（上昇・値上がり）の値は `<span class="chg-up">+◯◯</span>`、マイナス（下落・値下がり）の値は `<span class="chg-down">-◯◯</span>` で囲んで色分けする**（Markdownの表セル内でもこのHTMLはそのまま使える）。プラスマイナスがはっきりしない定性的な表現（「横ばい」「様子見」等）は無理に色付けしなくてよい。例:
  ```
...
2. **英語版**: `/Users/atsu/Desktop/AI/Claude/news-blog/_posts/YYYY-MM-DD-daily-briefing-en.md`。front matter は下記（`lang: en` と、日本語版と**同じ permalink**）。本文は日本語の直訳ではなく、**その日の米国ニュースを中心にした英語記事**。構成:
   ...
   - `## Compass Insight` **毎日必ず入れる固定コーナー（100〜150語）**。ブランド名を冠した看板コーナー。その日の相場・ニュースの流れを編集者視点で読み解き、短期・中長期の注目点を独自の言葉で。新しい未確認の事実は追加しない。投資助言はしない。この看板コーナーを記事前半に置く
   - `## Markets` 数行
   - `## Stocks in Focus` 任意
...
## 3.4 批判的セルフレビュー（必須・押す前の editor's check）
記事を書き終えたら、**書いた本人ではなく「疑り深い編集長」の目**で読み直す。具体的には:
1. 記事中で**最も影響の大きい主張を3つ**選ぶ（政権交代・選挙・勝敗・大きな数値など）
2. それぞれを**新しいWebSearchで改めて検証**する（執筆時に使った検索結果を信じ直すのではなく、別のクエリで裏を取り直す）
3. 食い違いがあれば修正。特に過去の失敗パターンに注意: **去年の季節ニュースの流用**（例: 参院選）／**スポーツの得点者・順序の取り違え**／**日付のずれ（就任日・発表日が「今日」なのか「明日」なのか）**
4. 修正した場合は、日本語版・英語版の両方に反映する
...
日本語版・英語版の記事を保存した後、push前に次を実行する。`assets/og/YYYY-MM-DD-daily-briefing.png` と `assets/og/YYYY-MM-DD-daily-briefing-en.png` が生成されたことを確認する:
...
- **【去年ニュースの流用禁止】** 検索結果には昨年以前の記事が混在する。特に**選挙・年中行事・季節イベント**（参院選/衆院選、記念日、大会など）は要注意。「参院選」は2025年7月（第27回）・次回は2028年で、**2026年には参院選はない**——このように、書こうとしている出来事が**本当に今年（当年）に起きているか**を、検索結果の日付（記事URLやタイムスタンプが当年か）で必ず確認する。当年の裏が取れないイベントは書かない。過去記事の使い回しと感じたら省く。
- 最後に、生成した記事タイトル（日英）、push結果、**ビルド結果（success/failure）と当日URLのHTTPステータス**を簡潔に報告する。

## 完了条件

次の3つがすべて満たされて初めて完了です。ひとつでも欠けている状態で報告を終えないでください。

1. 当日日付の記事ファイルが日本語版・英語版の2本とも `_posts/` に存在し、3.5の検証スクリプトが `VALIDATION OK` を返している
2. `git push` が成功し、GitHub Actions のビルドが `success` で終わっている
3. 当日URL（日本語版・英語版）がいずれも HTTP 200 を返している

ツールを一度も呼び出さずにこのタスクを終えた場合、それは実行されなかったということです。
